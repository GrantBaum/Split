...
namespace split {
    ...
    while(!atEnd()){
        ...
        //this one handles IDENTS and Keywords, which are words that are like var names and stuff
            ...
            if (KEYWORDS.count(str)) {
                t.type = Tok::KEYWORD; //if it is a keyword make it a keyword token
            } 
            else {
                t.type = Tok::IDENT; //otherwise it is a regular ident token
            }
            t.text = str;
            t.line = line_;
        ...
        //handles numbers
            ...
            advance(); //do i need to throw something here idk
        }
